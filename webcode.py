...
def update_profile():
	if 'lid' in session:
		...
		name = request.form['name']
		mail = request.form['mail']
		if (profile_db.EMAIL[profile_db.LOGIN_ID == session['lid']]).tolist()[0] != mail and mail in list(profile_db.EMAIL):
			return '''<script>alert('Sorry This Mail-ID is already in use');window.location='/my_profile'</script>'''
